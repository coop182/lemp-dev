...
# Drop all local 10.0.x.x host entries in '/etc/hosts'.
hosts-local-purge() {
  log-operation "$FUNCNAME" "$@"
  $SUDO sed -e 's#10\..*$##g' -i '/etc/hosts'
}

# Set up an IP as a DNS name server if not already present in 'resolv.conf'.
hosts-append() {
  log-operation "$FUNCNAME" "$@"
  grep "$1 $2" '/etc/hosts' >/dev/null || \
...
dependency-package-associate 'add-apt-repository' 'software-properties-common'
